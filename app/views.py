    ...
    if request.POST['role'] == "Candidate":
        ...
        if request.POST["role"] == "Company":
            ...
            if user1:
                ...
                if password == cpassword:
                    otp = randint(100000, 999999)
                    newuser1 = UserMaster.objects.create(
                        role=role, otp=otp, email=email, password=password
                    )
                    newcomp = Company.objects.create(
                        user_id=newuser1, firstname=fname, lastname=lname
                    )
                    return render(request, "app/otpverify.html", {"email": email})
...
def UpdateProfile(request, pk):
    ...
    if user.role == "Candidate":
        can = Candidate.objects.get(user_id=user)
        # breakpoint()
        can.country = request.POST[
            "country"
        ]  # first country belongs to database field and second field is belongs to html input name
